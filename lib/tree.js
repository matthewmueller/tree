...
class Tree {
  ...
  removeFile (node, options) {
    ...
    debug('removing file %s: %j', utils.relative(file.path), config)
    if (config.force) {
  ...
  dependenciesOf (node, options) {
    ...
    let file = this.getFile(id(node))
    debug('getting dependencies of %s: %j', utils.relative(file.path), config)
  ...
  dependantsOf (node, options) {
    ...
    let config = defaults(options, { recursive: false })
    let file = this.getFile(id(node))
    debug('getting dependants of %s: %j', utils.relative(file.path), config)
