...
const builtins = [
  // vinyl
  'cwd', 'base', 'path', 'history', 'stat', '_contents',
  // mako
  'tree'
]
...
class File extends Vinyl {
  ...
  constructor (params, tree) {
    ...
    if (typeof params === 'string') {
      ...
      super(params)
      // pick up custom properties
      Object.keys(params || {}).forEach(key => {
        if (builtins.indexOf(key) === -1 || key === 'history' || key === '_contents') {
          this[key] = clone(params[key])
        }
      })
    }
  ...
  clone (tree) {
    ...
    return new File(this, tree || this.tree)
  }
