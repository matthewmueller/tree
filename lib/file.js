...
class File {
   ...
   * @param {Boolean} timers  Whether or not to recreate the timers.
   * @return {File}
   */
  clone(tree, timers) {
    ...
    file.tree = tree;
    file.timers = timers ? new Map(file.timers.entries()) : new Map();
    file.timing = timers ? new Map(file.timing.entries()) : new Map();
    return file;
  }
}
